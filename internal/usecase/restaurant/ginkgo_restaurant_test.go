...
var _ = Describe("GinkgoRestaurant", func() {
	var usecase restaurant.Usecase
	...
	BeforeEach(func() {
		...
		usecase = restaurant.GetUsecase(menuRepoMock, orderRepoMock, userRepoMock)
	})
	...
	Describe("request order info", func() {
		Context("it gave the correct inputs", func() {
			...
			When("the requested orderID is not the user's", func() {
				BeforeEach(func() {
					orderRepoMock.EXPECT().GetOrderInfo(gomock.Any(), inputs.OrderID).Times(1).Return(model.Order{
						ID:            "valid_order_id",
