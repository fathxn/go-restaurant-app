	...
	tests := []struct {
	...
	}{
		{
			...
			initMock: func() (*sql.DB, sqlmock.Sqlmock, error) {
				...
				mock.ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "menu_items"`)).WillReturnRows(sqlmock.NewRows([]string{
					"name",
					"order_code",
					"price",
					"type",
...

		{
			...
			initMock: func() (*sql.DB, sqlmock.Sqlmock, error) {
				...
				mock.ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "menu_items"`)).WillReturnRows(sqlmock.NewRows([]string{
					...
					"price",
					"type",
				}).AddRow("Nasi Uduk", "nasduk", "15000", constant.MenuTypeFood))
...

		{
			...
			initMock: func() (*sql.DB, sqlmock.Sqlmock, error) {
				...
				mock.ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "menu_items"`)).WillReturnError(errors.New("mock error"))
